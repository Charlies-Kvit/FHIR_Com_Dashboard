<script lang="ts">
  ...
  import { Modal } from "../..";
  import { AddDashboard, EditDashboard } from "@/entities/dashboard";
  import LeftPanelEntry from "./LeftPanelEntry.svelte";
  import { AddAccount } from "@/entities/account";
  ...
  let classs = $derived.by(() => {
    if (is_open === undefined) return "";
      ...
      return (
        "fixed inset-0 z-50 " +
        (is_open ? "animate__slideInLeft" : "animate__slideOutLeft")
      );
  });
  $effect(() => {
    if (
      innerWidth > 1024 /* tailwindcss lg responsive width */ &&
      is_open !== undefined
    )
      is_open = undefined;
  });
...
<section class="row-span-15 col-span-1 box-content overflow-hidden">
  <nav
    class="bg-base-300 flex flex-col h-full px-6 py-9 transition-all animate__animated {classs} lg:static lg:!animate-none"
  >
    ...
    <div>
      <button
        class="rounded-full bg-[#5D5D5D]"
        onclick={() => (show_add = true)}
        ><span
          class="block i-material-symbols-add text-[32px] text-base-200 dark:text-base-content"
        ></span></button
      >
    </div>
  </nav>
...
<button
  class="fixed bottom-4 right-4 text-4xl lg:hidden z-50"
...
<Modal bind:open={show_edit_dashboard}>
  <EditDashboard
    dashboard={dashboard_to_edit!}
    onremove={() => (show_edit_dashboard = false)}
  ></EditDashboard>
</Modal>
<Modal bind:open={show_add_person}>
  <AddAccount
    dashboard={dashboard_to_edit!}
    onadd={() => (show_add_person = false)}
  ></AddAccount>
</Modal>
