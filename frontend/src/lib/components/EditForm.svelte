<script lang="ts">
    ...
    import DeleteConfirmation from "./Prompts/DeleteConfirmation.svelte";
  ...
  onMount(()=>{ (container.querySelector('input, select, button') as HTMLInputElement).focus() })
...
<article class="edit-form">
  ...
  <div bind:this={container} class="flex flex-col gap-6 min-w-[400px]">
    ...
    >
    {#if ondelete}
    <button class="link text-error self-end -mt-4" onclick={()=>delete_prompt = true}
      >Delete</button
    >
    {/if}
  </div>
...
<DeleteConfirmation {ondelete} oncancel={()=>delete_prompt = false}></DeleteConfirmation>
</Modal>
{/if}
<style lang="scss">
:global(.edit-form) {
