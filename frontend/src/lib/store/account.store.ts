...
export class Account {
  ...
  static from_object(
    ...
    return new Account(obj.avatar_url, obj.name, obj.email, obj.group_id, obj.id, obj.zulip_id)
  }
...
const createAccountsStore = () => {
    ...
    fetch('/api/accounts')
      .then(
        response => response.json()
          .then(
            result => set(result.accounts.map((v: Omit<Account, 'summary'>) => Account.from_object(v))
            )
          )
      )
  }
  ...
  const add_account = (account: Omit<Account, "id" | "summary">) => {
    fetch("/api/accounts", {
      method: "POST",
      ...
      if (res.ok) {
        ...
        res.json().then(
          (account: Omit<Account, 'summary'>) => {
            update(store => [...store, Account.from_object(account)])
          }
        )
      }
    });
  ...
  return { subscribe, add: add_account, update: update_account, remove: remove_account }
}
