...
const createAccountsStore = () => {
  ...
  const update_account = async (account_new: Omit<Account, "summary">) => {
    ...
    if (res.ok) {
      const new_account: Account = Account.from_object(await res.json());
      console.log("Update account with id ", new_account.id, new_account);
      update((store) => {
        console.log("Store updated");
        const next_store = store.filter((acc) => acc.id != new_account.id);
        return [...next_store, new_account];
      });
    }
  };
