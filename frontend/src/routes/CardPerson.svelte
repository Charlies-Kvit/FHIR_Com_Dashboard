<script lang="ts">
  ...
  import PersonSettings from "./PersonSettings.svelte";
  import type { Person } from "$lib/store.svelte";
  import type { Summary } from "$lib/types";
  import Bar from "./Bar.svelte";

  let { person } = $props<{ person: Person }>();

  let response: Summary[] = $state([
    {
      user_post_count: 25,
  ...
  let summary = $derived(response[active_summary]);
...
<div class="card bg-base-100 card-bordered border-base-300 flex flex-row p-6">
  ...
  >
    <Bar data={response} bind:selected={active_summary}></Bar>
  </div>
